...
if (isset($_GET['action'])) {
    ...
    switch ($_GET['action']) {
        case 'fetch_messages':
            ...
            // Get total count using db_config function
        ...
        case 'get_counts':
            ...
            if ($userId) {
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM v_user_sent WHERE user_id = :user_id");
                $stmt->execute([':user_id' => $userId]);
                $total = (int)$stmt->fetchColumn();
                
                $stmt = $pdo->prepare("SELECT COUNT(*) FROM v_user_sent WHERE user_id = :user_id AND label_id IS NOT NULL");
                $stmt->execute([':user_id' => $userId]);
                $labeled = (int)$stmt->fetchColumn();
            }
...
/**
 * Get user labels (simple wrapper around getLabelCounts)
 */
function getUserLabels($userEmail) {
    return getLabelCounts($userEmail);
}
